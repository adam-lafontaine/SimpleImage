...
namespace ns_unified_buffer_tests
{

using Buffer32 = img::DeviceBuffer32;
...
static bool push_elements_test()
{
    ...
    printf("create_unified_buffer()\n");
    result = cuda::create_unified_buffer(buffer, capacity);
    printf("capacity: %u\n", buffer.capacity_);
    printf("size: %u\n", buffer.size_);
    if (!result)
    {
...
static bool reset_test()
{
    ...
    printf("create_unified_buffer()\n");
    cuda::create_unified_buffer(buffer, n_elements);
    printf("capacity: %u\n", buffer.capacity_);
    printf("size: %u\n", buffer.size_);
...
{
    printf("\n*** unified_buffer tests ***\n");

    auto result = 
        ns_unified_buffer_tests::create_destroy_test() &&
        ns_unified_buffer_tests::push_elements_test() &&
        ns_unified_buffer_tests::pop_elements_test() &&
        ns_unified_buffer_tests::reset_test();

    if (result)
    {
        printf("unified_buffer tests OK\n");
    }
    return result;
}