...
void sub_view_host_to_device_test(img::View const& out)
{
    ...
    auto h_src = img::make_view_resized_from_file(CADILLAC_PATH, image, width, height, h_buffer);

    auto d_buffer = img::create_device_buffer32(width * height);
    auto d_src = img::make_view(width, height, d_buffer);

    img::copy_to_device(img::sub_view(h_src, r_top_left), img::sub_view(d_src, r_bottom_right));
    img::copy_to_device(img::sub_view(h_src, r_top_right), img::sub_view(d_src, r_bottom_left));
    img::copy_to_device(img::sub_view(h_src, r_bottom_left), img::sub_view(d_src, r_top_right));
    img::copy_to_device(img::sub_view(h_src, r_bottom_right), img::sub_view(d_src, r_top_left));

    img::copy_to_host(d_src, out);
